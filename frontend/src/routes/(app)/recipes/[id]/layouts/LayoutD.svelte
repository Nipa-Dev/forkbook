...
{#snippet extraInfo()}
  <div class="space-y-6">
    {#if recipe.notes?.length > 0}
      <div class="space-y-1">
        <span class="text-[10px] uppercase font-bold tracking-tighter block">
          Notes
        </span>
...
<article class="max-w-6xl mx-auto px-6 py-8">
  <div class="grid grid-cols-1 lg:grid-cols-[1fr_320px] gap-2 lg:gap-16">
    <aside class="order-first lg:order-last space-y-8 lg:sticky lg:top-8 h-fit">
      {#if recipe.image_url}
        <div class="rounded-xl overflow-hidden shadow-sm border">
          <img
            src={recipe.image_url}
            alt={recipe.title}
            class="w-full rounded-xl object-cover shadow-md aspect-video lg:aspect-square"
          />
        </div>
      {/if}
      ...
      <div class="space-y-6">
        ...
        <div class="pt-4 border-t space-y-6">
          ...
          <div class="hidden lg:block pt-4 border-t">
            {@render extraInfo()}
          </div>
        </div>
      </div>
    ...
    <div class="order-last lg:order-first space-y-12">
      <header class="border-b pb-6">
        ...
        {#if !recipe.description}
          <p class="text-lg text-muted-foreground leading-relaxed">
            This is very very cool text isn't this? A very nice recipe, and with
            very not hardcoded recipe description to fit the needs of my
            laziness. Also... would you like 3 page essay about this very recipe
            and hear my life story before actually getting to read the recipe? I
            won't even promise you a recipe, maybe this is all for ads??
            {recipe.description}
          </p>
        {/if}
      </header>
      ...
      <footer class="lg:hidden pt-8 border-t">
        {@render extraInfo()}
      </footer>
    </div>
  </div>
